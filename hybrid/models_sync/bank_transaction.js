...
const BankTransactionSyncData = async () => {
  try {
    const localUsers = await bank_transaction.findAll({ raw: true });
      ...
      if (cloudUser.length === 0) {
        await cloudDB.query(
          `INSERT INTO \`bank_transactions\` (
                id, account_list_id_bank_from, transaction_date, transaction_number, account_list_id_bank_to, module_from, description, amount, status, confirmed_by, amount_to_deduct, rate, orig_rate, createdAt, updatedAt, isDeleted
              ) VALUES (
                :id, :account_list_id_bank_from, :transaction_date, :transaction_number, :account_list_id_bank_to, :module_from, :description, :amount, :status, :confirmed_by, :amount_to_deduct, :rate, :orig_rate, :createdAt, :updatedAt, :isDeleted
              )`,
          {
            replacements: {
              id: user.id,
              account_list_id_bank_from: user.account_list_id_bank_from,
              transaction_date: user.transaction_date,
              transaction_number: user.transaction_number,
              account_list_id_bank_to: user.account_list_id_bank_to,
        ...
        console.log(
          `✅ Synced missing bank_transactions to cloud: ${user.account_list_id_bank_from} ${user.transaction_number} (${user.id})`
        );
      } else {
        ...
        if (needsUpdate) {
          ...
          await cloudDB.query(
            ...
            account_list_id_bank_from = :account_list_id_bank_from,
            transaction_date = :transaction_date,
            transaction_number = :transaction_number,
            account_list_id_bank_to = :account_list_id_bank_to,
          ...
          console.log(
            `🔁 Updated bank_transactions in cloud: ${user.account_list_id_bank_from} ${user.transaction_number} (${user.id})`
          );
        }
      }
    }
    ...
    for (const user of cloudUsers) {
      ...
      const localUser = await bank_transaction.findAll({
        where: { id: user.id },
      });
      ...
      if (localUser.length === 0) {
        ...
        console.log(
          `✅ Synced bank_transactions to local: ${user.account_list_id_bank_from} ${user.transaction_number} (${user.id})`
        );
      } else {
        ...
        if (needsUpdate) {
          ...
          console.log(
            `🔁 Updated bank_transactions in local: ${user.account_list_id_bank_from} ${user.transaction_number} (${user.id})`
          );
        }
      }
    }
...
module.exports = { BankTransactionSyncData };
