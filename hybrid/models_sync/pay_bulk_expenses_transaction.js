const { Sequelize } = require("sequelize");
const { localDB, cloudDB } = require("../connection.js");
// const { MasterList, cloudDB, Sequelize } = require("../db/models/associations"); // Adjust the import path as per your structure
// Import all models and manually attach them
const {
  PayBulkExpensesTransaction,
} = require("../../backend/db/models/associations.js");

const PayBulkExpensesTransactionSyncData = async () => {
  try {
    const localUsers = await PayBulkExpensesTransaction.findAll({ raw: true });

    /** ✅ STEP 1: Sync Local → Cloud **/
    for (const user of localUsers) {
      // console.log(
      //   `📝 Checking user in cloudDB: ${user.id} - ${user.based_currency} ${user.currency_name}`
      // );

      const cloudUser = await cloudDB.query(
        "SELECT * FROM `pay_bulk_expenses_transactions` WHERE id = :id",
        {
          replacements: { id: user.id },
          type: Sequelize.QueryTypes.SELECT,
        }
      );

      if (cloudUser.length === 0) {
        await cloudDB.query(
          `INSERT INTO \`pay_bulk_expenses_transactions\` (
                id, pay_bulk_id, expenses_id, createdAt,  updatedAt, isDeleted
              ) VALUES (
                :id, :pay_bulk_id, :expenses_id, :createdAt,  :updatedAt, :isDeleted
              )`,
          {
            replacements: {
              id: user.id,
              pay_bulk_id: user.pay_bulk_id,
              expenses_id: user.expenses_id,
              createdAt: user.createdAt,
              updatedAt: user.updatedAt,
              isDeleted: user.isDeleted,
            },
            type: Sequelize.QueryTypes.INSERT,
          }
        );
        console.log(
          `✅ Synced missing pay_bulk_expenses_transactions to cloud: ${user.transaction_number} ${user.status} (${user.id})`
        );
      } else {
        // Compare fields to check if update is needed
        const cloud = cloudUser[0];
        // const needsUpdate = cloud.updatedAt !== user.updatedAt;
        const needsUpdate =
          new Date(user.updatedAt).getTime() >
          new Date(cloud.updatedAt).getTime();

        if (needsUpdate) {
          //   console.log(`pumasok sa needsUpdate`);
          await cloudDB.query(
            `UPDATE pay_bulk_expenses_transactions SET
            pay_bulk_id = :pay_bulk_id,
            expenses_id = :expenses_id,
            updatedAt = :updatedAt,
            isDeleted = :isDeleted
            WHERE id = :id;`,
            {
              replacements: {
                id: user.id,
                pay_bulk_id: user.pay_bulk_id,
                expenses_id: user.expenses_id,
                updatedAt: user.updatedAt,
                isDeleted: user.isDeleted,
              },
              type: Sequelize.QueryTypes.UPDATE,
            }
          );
          console.log(
            `🔁 Updated pay_bulk_expenses_transactions in cloud: ${user.pay_bulk_id} ${user.expenses_id} (${user.id})`
          );
        }
      }
    }

    /** ✅ STEP 2: Sync Cloud → Local **/
    const cloudUsers = await cloudDB.query(
      "SELECT * FROM `pay_bulk_expenses_transactions`",
      {
        type: Sequelize.QueryTypes.SELECT,
      }
    );
    for (const user of cloudUsers) {
      // console.log(
      //   `📝 Checking user in localDB: ${user.id} - ${user.based_currency} ${user.currency_name}`
      // );

      const localUser = await PayBulkExpensesTransaction.findAll({
        where: { id: user.id },
      });

      if (localUser.length === 0) {
        await PayBulkExpensesTransaction.create({
          id: user.id,
          pay_bulk_id: user.pay_bulk_id,
          expenses_id: user.expenses_id,
          createdAt: user.createdAt,
          updatedAt: user.updatedAt,
          isDeleted: user.isDeleted,
        });

        console.log(
          `✅ Synced missing pay_bulk_expenses_transactions to local: ${user.pay_bulk_id} ${user.expenses_id} (${user.id})`
        );
      } else {
        const local = localUser[0];
        // const needsUpdate = cloud.updatedAt !== user.updatedAt;
        const needsUpdate =
          new Date(user.updatedAt).getTime() >
          new Date(local.updatedAt).getTime();

        if (needsUpdate) {
          await PayBulkExpensesTransaction.update(
            {
              id: user.id,
              pay_bulk_id: user.pay_bulk_id,
              expenses_id: user.expenses_id,
              updatedAt: user.updatedAt,
              isDeleted: user.isDeleted,
            },
            {
              where: {
                id: user.id,
              },
            }
          );

          console.log(
            `🔁 Updated pay_bulk_expenses_transactions in local: ${user.module_from} ${user.transaction_number} (${user.id})`
          );
        }
      }
    }
  } catch (err) {
    console.error("❌ Sync error:", err);
  }
};

module.exports = { PayBulkExpensesTransactionSyncData };
