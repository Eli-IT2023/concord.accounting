...
router.route("/getFixedAsset").get(async (req, res) => {
  try {
    ...
    switch (filterColumn) {
      ...
      case "total_cost":
        if (searchText !== null && searchText !== "") {
          fixedAssetWhereClause = {
            total_cost: sequelize.where(literal(`CAST (total_cost AS CHAR)`), {
              [Op.like]: `%${searchText}%`,
            }),
          };
        }
    ...
    let fixedAsset = await FixedAsset.findAll({
      include: [
        {
          model: Expenses,
          required: true,
    ...
    if (fixedAsset.length === 0) {
      ...
      fixedAsset = await FixedAsset.findAll({
        include: [
          {
            model: Expenses,
            required: true,
    ...
    if (fixedAsset) {
      const updatedFixedAsset = fixedAsset.map((asset) => {
        ...
        return {
          ...
          product_name: asset.expense.product_name,
          currency_name: asset.expense.currency.currency_name,
          currency_rate: currency_rate,
          total_cost: asset.total_cost,
          depreciated_amount: depreciated_amount,
          net_value: asset.total_cost - depreciated_amount,
          months_left:
            asset.static_months_to_pay -
            asset.fixed_asset_forecasts.filter((forecast) => forecast.isPaid)
              .length,
...
router.route("/updateFixedAsset").post(async (req, res) => {
  ...
  try {
    ...
    console.log("exx", getData.expense.transaction_id);
    ...
    if (fixedAsset) {
      ...
      const findExpenses = await Expenses.findOne({
        where: {
          id: expenses_id,
        },
        attributes: ["transaction_id"],
      ...
      await Activity_Log.create({
        ...
        Transaction ID: ${getData.expense.transaction_id} to ${findExpenses.transaction_id},
