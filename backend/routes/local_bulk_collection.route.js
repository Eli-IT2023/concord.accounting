...
router
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      // First, get the transactions with count for pagination
      ...
      // Calculate amount, discount, and subtotal for each transaction
      const transactionsWithCalculations = await Promise.all(
        transactions.map(async (invoice) => {
          const products = await SalesInvoiceTagProduct.findAll({
            where: {
              sales_invoice_id: invoice.sales_invoice_id,
              isDeleted: 0, // Assuming you want to exclude deleted products
            },
          });

          // Initialize totals
          let amount = 0; // TOTAL SUM (QUANTITY * UNIT PRICE)
          let discount = 0; // TOTAL SUM PER PRODUCT DISCOUNT
          let subtotal = 0; // TOTAL GROSS (AMOUNT - DISCOUNT)
          let W = "";

          // Calculate for each product
          products.forEach((product) => {
            const productAmount = product.quantity * product.unit_price;
            const productDiscount =
              productAmount * (product.discount_item / 100); // Assuming discount_item is percentage
            const productSubtotal = productAmount - productDiscount;
          ...
          // Get withholding tax from SalesInvoice (assuming it's stored there)
          const withholdingTax = invoice.withholding_tax || 0; // Use 0 if null/undefined

          console.log(invoice, "This is withholding tax");

          // Calculate final amount after withholding tax
          subtotal -= withholdingTax;

          // Return the original invoice data with calculated values
          return {
            ...invoice.toJSON(), // Convert Sequelize instance to plain object
            calculated_amount: parseFloat(amount.toFixed(2)),
            calculated_discount: parseFloat(discount.toFixed(2)),
            calculated_subtotal: parseFloat(subtotal.toFixed(2)),
          };
        })
      );

      console.log(
        transactionsWithCalculations,
        "THIS IS TRANSACTIONS WITH CALCULATIONS"
      );
      ...
      res.json({
        ...
        data: transactionsWithCalculations,
      });
    } catch (err) {
  ...
  try {
    ...
    if (isCreate) {
      ...
      for (const data of floatPayment) {
        ...
        // If payment is bank and check is issued
        // if (data.paymentMethod === "Bank" && data.checkNumber !== "") {
        //   await issued_check.create({
        //     account_list_id_issued_from: null,
        //     account_list_id_issued_to: data.subject3,
        //     amount: data.amount,
        //     transaction_number: "",
        //     module_from: data.subject1,
        //     transaction_date: data.issuedDate,
        ...
        //   });
        // }
        // If payment is bank but no check number (direct transaction)
        // else if (data.paymentMethod === "Bank" && data.checkNumber === "") {
        //   await bank_transaction.create({
        //     account_list_id_bank_from: null,
        //     account_list_id_bank_to: data.subject3,
        //     amount: data.amount,
        //     transaction_number: "",
        //     module_from: data.subject1,
        //     transaction_date: data.issuedDate,
      ...
      // const totalPayments = await BulkCollectionPayment.sum("amount", {
      //   where: {
      //     bulk_collection_id: isCreate.id,
      //   },
      // });
      ...
      //     {
      //       where: {
      //         sales_invoice_id: items.map((item) => item.transactionId),
      //       },
      //     }
      //   );
      // }
...
router.route("/update").post(async (req, res) => {
  try {
    let { items, collectionDate, floatPayment, id, removeIds, userLoggedID } =
      req.body;
...
router.route("/viewLocalBulkCollection").get(async (req, res) => {
  try {
    const data = await BulkCollection.findOne({
      where: {
        id: req.query.id,
      },
      include: [
...
router.route("/getTransactionsByCustomerLocal").get(async (req, res) => {
  try {
    const { customer_id, selected_currency_id, searchText, filterColumn } =
      req.query;
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const offset = (page - 1) * limit;
