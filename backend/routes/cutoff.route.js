...
router.route("/createCutoff").post(async (req, res) => {
  const { from, to, cutoff_name, userLoggedID } = req.body;
    ...
    if (allCutoff.length > 0) {
      ...
      if (!previousCutoff) {
        ...
        return res.status(404).json({
          message: `The cutoff for  ${
            monthName[Number(firstDayOfPreviousMonth(from).split("-")[1] - 1)]
          } ${
            firstDayOfPreviousMonth(from).split("-")[0]
...
router.route("/dateValidation").get(async (req, res) => {
  ...
  try {
    const data = await Cutoff.findAll({ where: { isDeleted: false } });
    const validation = data.some((item) => {
      return date >= item.from && date <= item.to;
    });
