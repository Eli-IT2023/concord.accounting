  ...
  const queryFunctions = {
    ...
    async getFixedAssets() {
      ...
      assets?.forEach((asset) => {
        totals.originalValues += asset.total_cost * asset.expense.rate;
        asset.fixed_asset_forecasts?.forEach((forecast) => {
          if (forecast.isPaid) {
            totals.cummulativeDiscount += forecast.amount * asset.expense.rate;
          } else {
            totals.impairment += forecast.amount * asset.expense.rate;
          }
        });
      });
