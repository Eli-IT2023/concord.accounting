...
router.route("/create").post(async (req, res) => {
  try {
    const { expensesTypeOne, description, userLoggedID } = req.body;
...
router.get("/getPaginatedExpensesOne", async (req, res) => {
  try {
      ...
      default:
        expensesOneWhereClause = {
          [Op.or]: [
            ...
            ...createDateTimeSearchConditions("expenses_one", searchText),
          ],
        };
        break;
    }
...
router.route("/updateExpenses1").put(async (req, res) => {
  try {
    const { expensesId, expensesTypeOne, description, userLoggedID } = req.body;
