...
router.route("/fetchPaginatedUserRole").get(async (req, res) => {
  try {
...
router.get("/fetchFilteredUserRoleData", async (req, res) => {
  try {
      ...
      switch (filterColumn) {
        ...
        default:
          userRoleWhereClause = {
            ...userRoleWhereClause,
            [Op.or]: [
              ...userRoleTableColumns.map((column) => ({
                [column]: {
                  [Op.like]: `%${searchText}%`,
                },
              })),
...
router.route("/getUserRoleData").get(async (req, res) => {
  try {
    const { filterColumn, searchText, page, limit } = req.query;
...
'${getData.col_rolename}' to '${roleName}'
'${getData.col_desc}' to '${roleDescription}'
`,
      });
