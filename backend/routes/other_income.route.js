...
router.route("/getCode").get(async (req, res) => {
  ...
  try {
    ...
    let newRefCode = `OI-${currentMonth}${time}${generateTwoNum}`;
  ...
  try {
    ...
    const findCutoff = await Cutoff.findOne({
      where: {
        [Op.and]: [
          {
            from: { [Op.lte]: incomeDate },
          },
          {
            to: { [Op.gte]: incomeDate },
          },
        ],
        isDeleted: false,
      },
        ...
        await accountlist_transaction_subject.create({
          ...
          type:
            ...
            "Liabilities Account"
              ...
              : "Debit",
          isTransferOnly: true,
          module_from: "Other Income",
          transaction_number: "Other Income",
          rate: 1,
        });
      }
    }
...
// Other Income Soft Delete route
router.route("/deleteOtherIncome/:id").put(async (req, res) => {
  const transaction = await sequelize.transaction();
  try {
    const { id } = req.params;

    const fetching = await OtherIncome.findOne({
      include: [
        {
          model: Other_Income_Payment,
          required: true,
    ...
    // If the OtherIncome record is approved, reverse the effect of each payment
    if (fetching.status === "Approved") {
      for (const item of fetching.other_income_payments) {
        await accountlist_sub3.decrement("amount", {
          by: item.amount,
          where: {
            id: item.account_list_sub3.id,
          },
          transaction,
        });
      }
    ...
    // Soft Delete the Other Income record
    const otherIncomeSoftDelete = await OtherIncome.update(
      {
        isDeleted: true,
      },
      {
    ...
    res.status(200).json({ message: "Other Income Successfully Deleted" });
  } catch (error) {
...
router.route("/otherIncomeBulkSoftDelete").put(async (req, res) => {
  const transaction = await sequelize.transaction();
  try {
    const { selectedOtherIncome } = req.body;

    // Loop through each selected Other Income ID
    for (const id of selectedOtherIncome) {
      // Fetch the OtherIncome record with its related payments and account info
      const fetching = await OtherIncome.findOne({
        include: [
          {
            model: Other_Income_Payment,
    ...
    await transaction.commit();
    res.status(200).json({
      message: `${selectedOtherIncome.length} other income deleted successfully`,
    });
  } catch (error) {
    if (transaction) await transaction.rollback();
    console.error(error);
