    ...
    if (isExist) {
      ...
      if (getData.currency_rate !== currencyRate) {
        ...
        if (currentCutoff) {
          ...
          if (accountUSD && accountUSD.length > 0) {
            ...
            if (mergedInvoice) {
              ...
              const actualTransactionAmount =
                mergedInvoice.amount * currencyRate;
...
router.get("/fetchPaginatedCurrency", async (req, res) => {
  try {
    ...
    switch (filterColumn) {
      ...
      case "currency_rate":
        if (searchText != "" && searchText != null) {
          ...
          const searchNumber = parseFloat(searchText.replace(/,/g, ""));

          if (!isNaN(searchNumber)) {
            // Create OR condition for both exact number and string pattern matching
            currencyWhereClause = {
              [Op.or]: [
                // Exact number match (accounting for floating point precision)
                {
                  currency_rate: {
                    [Op.between]: [
                      searchNumber - 0.0001,
                ...
                // String pattern matching for partial matches
                sequelize.where(literal(`CAST(currency_rate AS CHAR)`), {
                  [Op.like]: `%${searchText}%`,
                }),
              ],
            };
          } else {
            // Fall back to string search if not a valid number
            currencyWhereClause = {
        ...
        currencyWhereClause = {
          [Op.or]: currencyTableColumn.map((col) => {
            if (col === "currency_rate") {
              // Try to parse the search text as a number for currency_rate
              const searchNumber = parseFloat(searchText.replace(/,/g, ""));
              ...
              if (!isNaN(searchNumber)) {
                return {
                  [Op.or]: [
                    ...
                    sequelize.where(literal(`CAST(${col} AS CHAR)`), {
                      [Op.like]: `%${searchText}%`,
                    }),
                  ],
                };
