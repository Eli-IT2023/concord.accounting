  ...
  try {
    ...
    const totalIn = transferData
      ...
      .reduce((total, transfer) => {
        ...
        let convertedAmount = amount * (exchangeRate || 0);
    ...
    rows = rows.map((item) => {
      ...
      const currencyName =
        item.account_list_id_bank_froms?.currency?.currency_name ||
        item.account_list_id_bank_tos?.currency?.currency_name;
      return {
        ...item.toJSON(),
        exchangeRate,
        currencyName,
...
router.route("/confirmBankTransaction").post(async (req, res) => {
  try {
    ...
    if (description === "" || description !== "Transfer") {
      ...
      console.log(bankTransactionData.length);
      console.log(issuedCheckData.length);
      console.log(bankTransactionData);
      console.log(issuedCheckData);

      console.log(isAllOtherBankTransactionConfirmed);
      console.log(isAllOtherBankTransactionConfirmed);
      if (
        ...
        console.log("pasok");
        console.log(payableBulkTransactionData);
...
router.route("/v2/confirmBankTransaction").post(async (req, res) => {
  try {
    const {
      id,
      account_list_id_bank_from,
    ...
    if (description === "" || description !== "Transfer") {
      // Payable/PayableBulk: Get total amount to be paid, payable bulk id and identify all Payables for status update
      ...
      // Expenses: Get total amount to be paid and identify all Expenses for status update
      ...
      // Handles status update for "Expense", "Payable" and "PayableBulk"
