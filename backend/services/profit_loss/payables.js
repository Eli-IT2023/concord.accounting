...
const { Currency, PayableJournal } = require("../../db/models/associations");

async function getPayableData(date_to) {
  /* ---------------------------------------------------
   * 1. Fetch DEBITS
   * --------------------------------------------------- */
  const debits = await PayableJournal.findAll({
    attributes: [
      "id",
      "date",
      "currency_name",
   ...
   * 2. Fetch CREDITS
   * --------------------------------------------------- */
  const credits = await PayableJournal.findAll({
    attributes: [
      "id",
      "date",
      "currency_name",
