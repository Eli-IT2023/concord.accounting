...
async function getAccountsData(date_to) {
  /* ---------------------------------------------------
   * 1. Fetch DEBITS
   * --------------------------------------------------- */
  const debits = await accountlist_transaction_subject.findAll({
    attributes: [
      "id",
      "date",
      "currency_name",
    ...
    order: [["currency_name", "ASC"], -["date", "ASC"], ["id", "ASC"]],
    raw: true,
  });
   ...
   * 2. Fetch CREDITS
   * --------------------------------------------------- */
  const credits = await accountlist_transaction_subject.findAll({
    attributes: [
      "id",
      "date",
      "currency_name",
   ...
   * 5. Load SYSTEM RATES
   * --------------------------------------------------- */
  const currencies = await currency_sub.findAll({
