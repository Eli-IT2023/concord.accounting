...
const CreatePayable = () => {
  ...
  const [totalWeight, setTotalWeight] = useState(0); // amount useState
  ...
  const { register, control, setValue, watch } = useForm({
    defaultValues: {
      ...
      items: Array.from({ length: 5 }, () => ({ ...defaultItems })),
    },
  });
  ...
  const calculateTotalMoisture = () => {
    const totalMoisture = itemsValues.reduce((acc, item) => {
      ...
      if (item.moistureType === "%") {
        ...
        // Moisture amount = (Original Weight - Net Weight) × Unit Price
  ...
  const calculateNetWeight = (index) => {
    const weight =
      parseFloat(watch(`items.${index}.weight`)?.replace(/,/g, "")) || 0;
    const moisture =
      parseFloat(String(watch(`items.${index}.moisture`))?.replace(/,/g, "")) ||
      0;
    const unitPrice =
      parseFloat(watch(`items.${index}.unitPrice`)?.replace(/,/g, "")) || 0;
    const moisture_type = watch(`items.${index}.moistureType`);
    ...
    if (moisture_type === "%") {
      netWeight = Math.trunc(weight * (1 - moisture / 100));
    } else {
      netWeight = Math.trunc(weight - moisture / unitPrice);
    }
  ...
  useEffect(() => {
    ...
    // const calculateTotalNetWeightAmount = () => {
    //   const totalNetWeightAmount = itemsValues.reduce((acc, item) => {
    //     const parseValue = (val) => {
    //       const parsed = parseFloat(val?.toString().replace(/,/g, "") || 0);
    //       return isNaN(parsed) ? 0 : parsed;
    //     };

    //     const weightAmount = parseValue(item.net_weight);
    //     const unit_price = parseValue(item.unitPrice);
    //     return acc + weightAmount * unit_price;
    //   }, 0);
  ...
  const fetchLastCode = () => {
    axios
      ...
      .then((res) => {
        ...
        setValue("clientTransactionId", res.data);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const fetchProductData = (selectedOption) => {
    const vendorId = selectedOption;
    axios
  ...
  const fetchPayableProduct = (selectedOption) => {
    const vendorId = selectedOption;
    axios
  ...
  const fetchCurrency = async () => {
    try {
      ...
      const phpCurrency = res.data.find(
        (item) => item.id === "11111111-1111-1111-1111-111111111111"
      );

      // Manually Initialize currencyId and currencyRate as PHP in useForm
      // because defaultValues is not working
      setValue("currencyId", phpCurrency?.id);
      setValue("currencyRate", phpCurrency?.currency_rate);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const dateValidation = async (selectedDate, value, dateLabel) => {
    try {
  ...
  const add = async (e) => {
    ...
    const currentCurrencyRate = watch("currencyRate");
    // const weighingFee =
    //   parseFloat(watch("weighingFee")?.replace(/,/g, "")) || 0;
    ...
    if (form.checkValidity() === false) {
      ...
      dateValidation(watch("purchaseDate"), "purchaseDate", "Purchase Date");
      ...
      swal({
        ...
        if (confirmed) {
          ...
          formData.append("currencyRate", currentCurrencyRate || 1);
          formData.append(
            "dueDate",
  ...
  //commented kasi may prefered ang calculation ng moisture to net weight sa ylluja
  ...
  const calculateTotalMoistureInKilo = itemsValues.reduce((total, value) => {
    // Helper to safely parse numbers with comma removal
    const parseValue = (val) => {
      const parsed = parseFloat(val?.toString().replace(/,/g, "") || 0);
      return isNaN(parsed) ? 0 : parsed;
    };
  ...
  // Product Option for dropdown select
  ...
  const filteredProductOptions = (index) => {
    const productIdList = itemsValues
      .filter((_, i) => i !== index)
      .map((item) => item.product_vendor_id);
  ...
  // Fetch Vendor then set the currency and currency rate based on selected vendor
  const fetchVendorCurrencyId = async (selectedOption) => {
    try {
  ...
  // useEffect(() => {
  //   if (!watch("items").some((item) => parseFloat(item.moisture) > 100)) {
  //     if (totalAmount < 0 && String(totalAmount).length < 19) {
  //       const wrapper = document.createElement("div");
  ...
  const selectProductWidth = Math.max(
    ...itemsValues.map((item) => item.productName.length)
  );
      ...
      <Form noValidate validated={validated} onSubmit={add}>
        ...
        <div className="row mx-auto p-2 mt-2">
          ...
          <div className="col-sm mb-2">
            <label htmlFor="clientTransactionId">Transaction ID</label>{" "}
            <span className="text-danger">*</span>
            <input
              type="text"
              id="clientTransactionId"
              {...register("clientTransactionId")}
              className="form-control p-2"
              required
            />
          </div>
            ...
            {/* <select
              ...
              onMouseDown={(e) => {
                if (vendors.length === 0) {
                  ...
                  wrapper.classList.add("center-swal-text");
                  wrapper.innerHTML =
                    "No vendors found. Please create one first.";
                  swal({
                    icon: "warning",
            ...
            {/* <Select
              ...
              {...register("vendorId", {
                onChange: async (selectedOption) => {
                  fetchProductData(selectedOption);
                  fetchPayableProduct(selectedOption);
                  await fetchVendorCurrencyId(selectedOption);
                },
              })}
              onMenuOpen={() => {
                if (vendors.length === 0) {
              ...
              menuIsOpen={vendors.length === 0 ? false : undefined}
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select
                  ...
                  options={vendorOptions}
                  value={
                    vendorOptions.find((opt) => opt.value === field.value) ||
                    null
                  }
                  onChange={async (selectedOption) => {
        ...
        <div className="row mx-auto p-2">
          ...
          <div className="col-sm mb-2">
            ...
            {/* <div className="position-relative">
              <DatePicker
                ...
                onChange={(date) => {
                  ...
                  dateValidation(date, "dueDate", "Deadline");
                }}
            ...
            <CustomDatePicker
              label={"Deadline"}
              field={"dueDate"}
              selected={watch("dueDate") ? new Date(watch("dueDate")) : ""}
              handleDateChange={(date) => {
                const purchaseDate = watch("purchaseDate");
                if (
                  validateDueDateGreaterThanPurchaseDate(date, purchaseDate)
                ) {
                  setValue("dueDate", date, { shouldValidate: true });
                }
              }}
            ...
            <div className="mt-5">
              ...
              {/* <div className="position-relative">
                <DatePicker
                  ...
                  onChange={(date) => {
                    ...
                    dateValidation(date, "purchaseDate", "Purchase Date");
                  }}
                  dateFormat="MMM/dd/yyyy"
                  className="form-control p-2"
                  customInput={<CustomInput />}
              ...
              <CustomDatePicker
                ...
                field={"purchaseDate"}
                selected={
                  watch("purchaseDate") ? new Date(watch("purchaseDate")) : ""
                }
                handleDateChange={(date) => {
                  setValue("purchaseDate", date, { shouldValidate: true });
                  dateValidation(date, "purchaseDate", "Purchase Date");
                ...
                {fields.map((item, index) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      ...
                      <Controller
                        name={`items.${index}.product_vendor_id`}
                        control={control}
                        rules={{ required: true }}
                        render={({ field }) => (
                          <Select
                            ...
                            menuIsOpen={
                              product.length === 0 ? false : undefined
                            }
                            menuPortalTarget={document.body}
                            placeholder="Select Product"
                            styles={selectCustomStyles(
                              watch(`items.${index}.product_vendor_id`),
                              validated,
                              "0",
                              selectProductWidth
                            )}
                            isDisabled={!watch("vendorId")}
                            isSearchable
                            // required
                          />
                        )}
                        ...
                        <input
                          ...
                          {...register(`items.${index}.moisture`, {
                            onChange: (e) => {
                              ...
                              // Auto-clear default "0" when user starts typing
                              if (
                                value.length === 2 &&
                                value[0] === "0" &&
                                value[1] !== "."
                              ) {
                                value = value[1]; // Keep only the new digit
                        ...
                        {...register(`items.${index}.net_weight`, {
                          onChange: (e) => {
                            ...
                            if (
                              ...
                              value = value[1]; // Keep only the new digit
                              e.target.value = value;
                              setValue(`items.${index}.net_weight`, value);
                            }

                            calculateMoisture(index, value || 0);
                          },
                        })}
                        maxLength={15}
                        className="form-control form-control-sm p-2"
                    ...
                    <td>
                      ...
                      <div className="d-flex flex-column">
                        ...
                        {watch(`items.${index}.productId`) && (
                          <div className="">
                            {watch(`items.${index}.supplier_price`) >
                            watch(`items.${index}.unitPrice`) ? (
                              <span className="text-primary">
                                Below Supplier Price (
                                {parseFloat(
                                  watch(`items.${index}.supplier_price`) || 0
                                ).toLocaleString(
                                  "en-US",
          ...
          <div className="col-sm border-end">
            <div className="w-100 d-flex flex-row justify-content-between p-2">
              ...
              <span className="text-secondary">
                {calculateTotalWeightInKilo.toLocaleString("en-US")} (Kg)
              </span>
            </div>
            {/* <div className="w-100 d-flex flex-row justify-content-between p-2">
            ...
            <div className="w-100 d-flex flex-row justify-content-between p-2">
              ...
              <span className="text-danger">
                {calculateTotalMoistureInKilo.toLocaleString("en-US")} (Kg)
              </span>
            </div>
            <div className="w-100 d-flex flex-row justify-content-between p-2">
              <span>Total Net Qty :</span>
              <span className="text-danger">
                {calculateTotalNetWeightInKilo.toLocaleString("en-US")} (Kg)
              </span>
            </div>
          </div>
          <div className="col-sm">
