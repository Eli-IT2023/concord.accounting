...
import ModalContentPaymentTransactions from "./modalLocalPOtransaction";
import { compactNumberFormat } from "../../../utils/numberFormatter";
import CurrencySelector from "../../../components/CurrencySelector";

const OldLocalPurchase = ({ authrztn }) => {
  ...
  const localPayableTotalPages = localStorage.getItem("Local-Payable");
  const validPage = useValidPage(localPayableTotalPages);
  ...
  const pagination = useServerPagination(
    BASE_URL + "/payable/fetch_local_payable_bulk",
    10,
    {},
    validPage
  );
  ...
  const fetchData = (cutOff) => {
    ...
    setIsLoading(false);
    // axios
    //   .get(BASE_URL + "/payable/fetch_local_payable_bulk", {
    //     params: {
    //       startDate: cutOff?.from,
    //       endDate: cutOff?.to,
    //       filterColumn,
  ...
  useEffect(() => {
    ...
    if (pagination.data.length > 0) {
      localStorage.setItem("Local-Payable", pagination.totalPages);
    }
  ...
  const handleRowClick = (data) => {
    navigate(
      `/Purchases/view-bulk-payable/local/${data.id}?page=${pagination.currentPage}`
    );
  };
  ...
  return (
    <div className="h-100 w-100 border bg-white custom-container">
      ...
      ) : authrztn.includes("LocalPurchase-View") ? (
        <>
          <div className="w-100 p-2 d-flex flex-row justify-content-between">
            <div className="d-flex flex-column title-custom">
              <span className="fs-3 text-uppercase">Local Purchase</span>
              <span>LOCAL ACCOUNTS PAYABLE</span>
            </div>
            <div className="d-flex flex-row align-items-center justify-content-end mx-2">
              <span className="">
                ...
                {authrztn.includes("LocalPurchase-Add") && (
                  <Link
                    to={`/Purchases/bulk-payable/${"local"}`}
          ...
          >
            ...
            <Modal.Body>
              <ModalContentPaymentTransactions
...
export default OldLocalPurchase;
