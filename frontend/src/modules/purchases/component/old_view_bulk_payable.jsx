...
import LocalOverseasPDF from "../Sub Folder/LocalOverseasPDF";
...
const ViewBulkPayable = ({
        ...
        setPayableList((prev) => {
          const newPayables = newItems.map((item) => {
            ...
            return {
              ...
              id: item.id || "",
              transactionId: item.transaction_id || "",
              clientTransactionId: item.client_transaction_id,
              invoiceDate: item.createdAt || "",
              purchaseDate: item.purchaseDate || "",
              description: item.description || "",
              dueDate: item.due_date || "",
  ...
  const handleRejected = async () => {
    ...
    if (confirmed) {
      try {
        const res = await axios.post(`${BASE_URL}/payable/updateStatus`, {
          id,
          payableDate,
          status: "Rejected",
          payableList,
                ...
                {payableList.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      <input
                        className="form-control"
                        value={item?.payable?.client_transaction_id || ""}
                        type="text"
                        readOnly
                      />
                    </td>
                  ...
                  <div className="col-sm mb-2">
                    <div className="w-100 d-flex flex-row justify-content-between p-3 mt-3 total-amount-container align-items-center rounded">
                      ...
                      <span className="text-white text-underline">
                        {currencyName}{" "}
                        {Math.max(0, parseFloat(balance)) == 0
                          ? "0.00"
                            ...
                            <div className="d-flex flex-column gap-2">
                              ...
                              <div>
                                ...
                                <div>
                                  ...
                                  >
                                    Action is prohibited as the Transaction date
          ...
          <PDFDownloadLink
            document={
              <LocalOverseasPDF
                id={id}
                module={module}
                currencyId={payableBulk.currency_id}
              />
            }
            fileName={`${
              module.charAt(0).toUpperCase() + module.slice(1)
            } Purchase - ${transactionNumber}.pdf`}
          >
            <OverlayTrigger
              placement="right"
              overlay={
                ...
                >
                  ...
                  <strong className="text-danger">
                    ...
                    {module.slice(1)}&nbsp;Purchase - {transactionNumber}.pdf"
                  </strong>
                </Tooltip>
              }
              delay={300}
