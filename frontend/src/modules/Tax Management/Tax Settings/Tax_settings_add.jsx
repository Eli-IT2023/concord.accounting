...
function TaxSettingsAdd() {
  ...
  const handleFormCreate = async (e) => {
    ...
    if (parseFloat(applicableRates.replace(/,/g, "")) > 100) {
      swal({
        icon: "error",
        title: "Invalid Rate",
        text: "Applicable rate cannot exceed 100%.",
      });
      setValidated(true);
      return;
    }
            ...
            .then((response) => {
              if (response.status === 200) {
                swal({
                  ...
                  setValidated(false);
                  navigate("/tax-management/tax-settings");
                });
    ...
    <div className="h-100 w-100 bg-white">
      <div className="w-100 d-flex flex-row justify-content-between align-items-center p-3">
        <div className="d-flex flex-column">
          <span className="fs-3">
            <button
              onClick={() => navigate("/tax-management/tax-settings")}
              className="text-dark border-0"
              style={{ background: "none" }}
            >
              <i className="bx bx-arrow-back"></i>
            </button>
            <span className="mx-2">TAX SETTINGS</span>
          </span>
        </div>
      </div>
