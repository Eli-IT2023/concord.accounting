...
const CountingCreate = ({ authrztn }) => {
  ...
  const handleUpdate = (e) => {
    ...
    const formattedItemList = itemList.map((item) => ({
      ...
      actual_price: parseFloat(String(item.actual_price).replace(/,/g, "")),
    }));
  ...
  const handleActualAmountChange = (productId, newCount, actualAmount) => {
    if (newCount == ".") {
    ...
    setItemList((prevList) =>
      prevList.map((item) => {
        ...
        const isActualCount = actualAmount === "actual_count";
        const isMatchingProduct = item.product_id == productId;

        // If the actual count is 0, actual price will be set to 0.
        if (isMatchingProduct && isActualCount && isValueZero) {
          actualPriceRef.current.disabled = true; // disable actual price input field
          return {
            ...item,
            [actualAmount]: formattedValue,
            actual_price: "0",
          };
        }

        if (actualPriceRef.current.disabled) {
          actualPriceRef.current.disabled = false;
        }

        return isMatchingProduct
          ? { ...item, [actualAmount]: formattedValue }
          : item;
      })
    );
  };
      ...
      cell: (row) => (
        <input
          ...
          onChange={(e) => {
            ...
            handleActualAmountChange(row.productId, rawValue, "actual_count");
          }}
          min={1}
    ...
    },
    {
      ...
      selector: (row) => (
        <input
          ...
          className="form-control"
          onInput={onInputFloat}
          value={row.actualPrice?.toLocaleString("en-US", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}
          onKeyDown={(e) => {
          ...
          onChange={(e) => {
            ...
            if (rawValue === "") rawValue = "0";
            handleActualAmountChange(row.productId, rawValue, "actual_price");
          }}
          ref={actualPriceRef}
          min={1}
          disabled={!authrztn.includes("InventoryCounting-Edit")}
      ...
      if (confirmed) {
        const data = {
          ...
          itemList: itemList.map((item) => ({
            ...
            actualPrice:
              parseFloat(String(item.actual_price).replace(/,/g, "")) || 0,
            transaction_id: inventoryCounting.transaction_id,
          })),
          transactionId: inventoryCounting.transaction_id,
          inventoryCountingId: id,
          userLoggedID,
        };
        axios
          .post(`${BASE_URL}/inventoryCounting/transactions/approve`, data)
          .then((res) => {
            if (res.status === 200) {
              swal({
  ...
  const CustomInput = React.forwardRef(
    ({ value, onClick, generateYears }, ref) => (
      <input
        ...
        required
        disabled={!authrztn.includes("InventoryCounting-Edit") || !isEditable}
      />
    )
  );
          ...
          <div className="row mx-auto">
            <div className="col-sm mb-3">
              ...
              <input
                ...
                readOnly
                value={inventoryCounting.transaction_id || ""}
              />
            </div>
              ...
              {/* <div className="position-relative">
                <DatePicker
                  ...
                  onChange={(date) => {
                    ...
                    dateValidation(date, setCountingDate, "Counting Time");
                  }}
