...
const CountingCreate = () => {
  ...
  const [transactionId, setTransactionId] = useState("COUNT-2024190330001");
  ...
  const handleActualCountChange = (productId, newValue, warehouseName) => {
    if (newValue == ".") {
      setItemList((prevData) =>
        prevData.map((row) =>
          row.productId === productId && row.warehouseName === warehouseName
            ? { ...row, actualCount: prevData + "." } // Update actual count
            : row
        )
      );
    }
    ...
    setItemList((prevData) =>
      prevData.map((row) =>
        row.productId === productId && row.warehouseName === warehouseName
          ? { ...row, actualCount: formattedValue } // Update actual count
          : row
      )
    );
      ...
      cell: (row) => (
        <input
          ...
          onChange={(e) => {
            ...
            handleActualCountChange(row.productId, rawValue, row.warehouseName);
          }}
          min={0}
          required
        />
        ...
        return (
          <div className="custom-checkbox">
            <input
              ...
              onChange={() => handleSelectRow(row.productId, row.warehouseName)}
        ...
        <Modal.Body>
          ...
          <div className="w-100 mt-4 container-fluid">
            <DataTable
              ...
              data={filteredProductList.map((data, i) => ({
                key: i,
                warehouseName: data.warehouse.name,
                productId: data.product_id,
                productName: data.product_list.product_name,
