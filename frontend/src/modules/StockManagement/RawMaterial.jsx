...
function RawMaterial() {
  ...
  const columns = [
    ...
    },
    {
      name: "Name",
      selector: (row) => (
        <div title={row.productName} className="text-center">
          {row.productName}
        </div>
      ),
      width: "24rem",
  ...
  const handleSearch = (e) => {
    ...
    if (
      !raw.startsWith("0") &&
      userData.find((item) => {
        const candidates = [item.totalPrice, item.averagePrice, item.stock];
