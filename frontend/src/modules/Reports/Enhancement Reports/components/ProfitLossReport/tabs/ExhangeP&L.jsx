...
const ExchangePL = ({ authrztn }) => {
  ...
  const pagination = useServerPagination(
    `${BASE_URL}/profit_loss/getJournalv2Data-Exchange`,
    10,
    {
  ...
  const getTotalGainLossInfos = () => {
    axios
      .get(BASE_URL + "/profit_loss/getTotalGainLossInfos-exchange", {
        params: {
          date_from: dateRangefilter?.from,
          date_to: dateRangefilter?.to,
        },
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="px-4 py-3">
                      <span className="fw-medium">{r.currency_name_from}</span>
                      <i className="fas fa-arrow-right mx-2 text-muted small"></i>
                      <span className="fw-medium">{r.currency_name_to}</span>
                    </td>

                    <td className="px-4 py-3 text-end fw-medium">
                      {formatNumber(r.amount_from, r.currency_name_from)}
                    </td>
                    <td className="px-4 py-3 text-end text-muted">
                      {formatNumber(r.received_rate, "PHP")}
                    </td>
                    <td className="px-4 py-3 text-end text-muted">
                      {formatNumber(r.exchange_rate, "PHP")}
                    </td>

                    <td className="px-4 py-3 text-end fw-medium">
                      {formatNumber(r.amount_to, r.currency_name_to)}
                    </td>
                    ...
                    <td className="px-4 py-3 text-end">
                      <span
                        ...
                        style={{
                          backgroundColor:
                            r.profitLossPHP >= 0 ? "#dcfce7" : "#fee2e2",
                          color: r.profitLossPHP >= 0 ? "#16a34a" : "#dc2626",
                          fontWeight: "600",
                        }}
                      >
                        {r.profitLossPHP >= 0 ? "+" : "-"}{" "}
                        {formatNumber(Math.abs(r.profitLossPHP), "PHP")}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-end">
                      <span
                        ...
                        style={{
                          backgroundColor:
                            r.percentPL >= 0 ? "#dcfce7" : "#fee2e2",
                          color: r.percentPL >= 0 ? "#16a34a" : "#dc2626",
                        }}
                      >
                        {r.percentPL >= 0 ? "+" : "-"}{" "}
                        {Math.abs(r.percentPL).toFixed(2)}%
                      </span>
                    </td>
                  </tr>
                ))}
