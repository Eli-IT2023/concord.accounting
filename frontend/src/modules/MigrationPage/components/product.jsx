...
const Product_Migration = ({ lastDate }) => {
      ...
      complete: function (results) {
        ...
        if (!isFileTemplateValid(cleanedData, requiredColumns)) {
          swal({
            ...
            text: "Your file doesn't match the expected template for Product Migration. Please download the sample file and try again.",
          }).then(() => (fileRef.current.value = ""));
  ...
  const handleSendToServer = async () => {
    ...
    try {
      await axios
        .post(BASE_URL + "/migration/product_data_migrate", {
          data: csvData,
          lastDate,
        })
        .then((res) => {
          if (res.status === 200) {
            swal({
              ...
              text: "Your product data has been migrated successfully.",
            });
          }
        });
    } catch (error) {
  ...
  const handleDownload = () => {
    ...
    const csvContent = [
      "Product Code,Product Name,Product Category,Product Threshold,Inventory Stock / Quantity,Product Price,Unit of Measure",
      "0000001,Sample Product,Finish Product,0,1000,10.12,Pieces",
      "0000002,Sample Product2,Raw Materials,1,2000,10,Kilogram",
      "0000003,Sample Product3,Consumables,2,3000,23,Kilogram",
      "0000004,Sample Product4,Consumables,2,3000,23,Pieces",
    ].join("\n");
    ...
    <div>
      <div className="row p-2 mx-auto align-items-end">
        <div className="col-sm-8">
          {" "}
          <Form.Label className="fw-bold">Migrate Product's Data</Form.Label>
          <Form.Control
            type="file"
            accept=".csv"
...
export default Product_Migration;
