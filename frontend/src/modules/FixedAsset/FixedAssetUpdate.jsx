...
const FixedAssetUpdate = ({ authrztn }) => {
  ...
  const [selectedFixedAsset, setSelectedFixedAsset] = useState([]);

  const [modalAllowZeroDepreciation, setModalAllowZeroDepreciation] =
    useState(false);
  const isAllowZeroDepreciation = monthsToPay == 0 && depreciationAmount == 0;
  const [editModeAllowZeroDepreciation, setEditModeAllowZeroDepreciation] =
    useState(false);
  ...
  const getApprovedForecast = () => {
    axios
      ...
      .then((res) => {
        ...
        const { fixedAssetForecast, paidItemCount, totalCost } = res.data;
        const sortedForecastDate = fixedAssetForecast?.sort(
          (a, b) => new Date(a.date) - new Date(b.date)
        );
        setApprovedForecast(sortedForecastDate);

        const total = fixedAssetForecast.reduce(
          (acc, curr) => (curr.isPaid ? acc + curr.amount : acc),
          0
        );
        setApprovedForecastValue(totalCost - total);
        setPaidCount(monthsToPay - paidItemCount); // Count of rows where isPaid is true
      });
  };
  ...
  const reloadData = () => {
    axios
      ...
      .then((res) => {
        ...
        setCurrencyName(res.data.currency.currency_name);
        ...
        setCurrencyId(res.data.currency_id);
        // setUnitPrice(res.data.unit_price);

        setEditModeAllowZeroDepreciation(
          res.data.static_months_to_pay === 0 &&
            res.data.depreciation_amount === 0
        );
      });
  };
  ...
  const calculateForecast = () => {
    ...
    // If zero depreciation is allowed, don't generate forecast
    if (modalAllowZeroDepreciation) {
      setForecast([]);
      setShow(true);
      return;
    }
  ...
  const handleCheckForecast = () => {
    // Set modal checkbox based on edit mode checkbox state instead of values
    setModalAllowZeroDepreciation(editModeAllowZeroDepreciation);
    calculateForecast();
    setShow(true);
  };

  const handleModalCheckboxChange = (checked) => {
    setModalAllowZeroDepreciation(checked);

    if (checked) {
      // Set values to 0 when checkbox is checked (UI only)
      setMonthsToPay(0);
      setDepreciationAmount(0);
      setForecast([]);
    } else {
      // Recalculate values when checkbox is unchecked
      handleMonthsToPay(monthsToPay || 1);
    }
  };
  ...
  const openModal = async (e) => {
    ...
    if (form.checkValidity() === false) {
      ...
      // Modified validation - only show alert if checkbox is NOT checked
      if (
        monthsToPay == 0 &&
        depreciationAmount == 0 &&
        !editModeAllowZeroDepreciation
      ) {
        swal({
          icon: "warning",
      ...
      // Modified validation - only check if checkbox is NOT checked
      if (Math.trunc(monthsToPay) === 0 && !editModeAllowZeroDepreciation) {
        swal({
          icon: "warning",
  ...
  const handleAmountChange = (value, setState) => {
    if (value === ".") {
      setState((prev) => (prev.includes(".") ? prev : prev + "."));
      return;
    }
  ...
  const handleEditModeCheckboxChange = (checked) => {
    setEditModeAllowZeroDepreciation(checked);
    // Also update the modal state
    setModalAllowZeroDepreciation(checked);

    if (checked) {
      // Set values to 0 when checkbox is checked (UI only, no DB update)
      setMonthsToPay(0);
      setDepreciationAmount(0);
    } else {
      // Recalculate values when checkbox is unchecked
          ...
          .then((res) => {
            if (res.status === 200) {
              swal({
                ...
                reloadData();
                setEdit(false);
                // Update edit mode checkbox after successful save
                setEditModeAllowZeroDepreciation(
                  formatMonthsToPay == 0 && formatDepreciationAmount == 0
                );
              });
            } else {
              swal({
  ...
  return (
    <div className="h-100 w-100 border bg-white custom-container">
            ...
            <div className="card text-center">
              <div className="card-body">
                ...
                <p
                  ...
                  onClick={handleCheckForecast}
                >
                  {paidCount.toLocaleString("en-US", {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}
                </p>
            ...
            <div className="col-sm mb-2">
              ...
              <div className="input-group mb-2">
                <input
                  ...
                  // }}
                  onChange={(e) => {
                    if (!editModeAllowZeroDepreciation) {
            ...
            <div className="col-sm mb-2">
              ...
              <div className="input-group mb-2">
                ...
                <input
                  type="text"
                  required
                  readOnly={!edit || isAllowZeroDepreciation}
                  className="form-control p-3"
                  id="inlineFormInputGroup"
                  placeholder="0.00"
              ...
              <div className="form-check">
                <input
                  ...
                  checked={
                    edit
                      ? editModeAllowZeroDepreciation
                      : isAllowZeroDepreciation
                  }
                  onChange={(e) => {
                    if (edit) {
                      handleEditModeCheckboxChange(e.target.checked);
                    }
                  }}
                  disabled={!edit}
                />
                ...
                >
                  Zero depreciation amount allowed
                </label>
              </div>
            </div>
          </div>
      ...
      >
        ...
        <Modal.Body>
          {/* Show message when zero depreciation is allowed */}
          {modalAllowZeroDepreciation ? (
            <div className="alert alert-info">
              <i className="fa-solid fa-info-circle me-2"></i>
              This asset has zero depreciation amount. No forecast will be
              generated.
            </div>
          ) : (
            // Show forecast only when zero depreciation is not allowed
            <>
              {status === "Approved"
                          ...
                          >
                            {date.isPaid ? (
                              ...
                              !modalAllowZeroDepreciation && (
                                <button
                                  onClick={() => {
                                    setSelectedFixedAsset((prev) => [
                                      ...prev,
                                      date,
                                    ]);
                                  }}
                                  disabled={
                                    selectedFixedAsset.includes(date) ||
                                    date.isCreated
                                  }
          ...
          <div className="d-flex justify-content-between">
            {selectedFixedAsset.length > 0 && !modalAllowZeroDepreciation && (
              <>
                <span className="mt-2">
                  Selected: {selectedFixedAsset.length}
                </span>
                <div>
                  <button
                    className="btn btn-success"
                    onClick={() => {
                      const totalAmount = selectedFixedAsset.reduce(
                        (acc, value) => {
                      ...
                      navigate(
                        `/accounting/add-expenses/${currencyId}/${totalAmount}`,
                        {
                          state: {
                            ids: selectedFixedAsset.map((item) => item.id),
                          },
                        }
                      );
                    }}
                ...
                <div className="d-flex flex-row justify-content-between">
                  ...
                  {authrztn.includes("FixedAssets-Approve") && (
                    <div className="d-flex flex-wrap gap-3">
                      ...
                      <div className="d-flex flex-row-reverse gap-3 justify-content-end">
                        <Button
                          variant="success"
                          type="button"
                          onClick={() => {
