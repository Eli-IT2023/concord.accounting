...
const CreateRbac = ({ authrztn }) => {
  ...
  const fetchUserRoleData = async () => {
    try {
      await axios
        .get(BASE_URL + `/userRole/fetchUserRoleDataToEdit`, {
          params: {
            id,
          },
        })
        .then((res) => {
          setroleName(res.data.col_rolename);
          setroleDescription(res.data.col_desc);
  ...
  const handleCheckboxChange = (event) => {
    ...
    } else if (id === "LabelManagement-View") {
      if (!checked) {
        updatedItems = checkedItems.filter(
          (item) => !item.id.startsWith("LabelManagement-")
        );
      } else {
        // If checking LabelManagement-View, just add it
        updatedItems = [...checkedItems, { id }];
      }
          ...
          axios
            ...
            .then((res) => {
              if (res.status === 200) {
                swal({
                  title: "Success",
                  text: "Role access successfully updated",
                  icon: "success",
                  button: "OK",
                }).then(() => {
                  navigate(
                    `/settings/access-control?page=${searchParams.get("page")}`
                  );
                  window.scrollTo(0, 0);
                });
  ...
  return (
    <fieldset
      {...(authrztn.includes("RBAC-Edit") ? {} : { disabled: true })}
      className="w-100 border bg-white p-3 h-100"
    >
