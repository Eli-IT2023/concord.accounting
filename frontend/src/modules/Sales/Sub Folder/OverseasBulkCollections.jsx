...
const OverseasBulkCollection = () => {
  ...
  const [overseasCustomerData, setOverseasCustomerData] = useState([]);
  ...
  const fetchTransactionCode = () => {
    axios
      .get(BASE_URL + "/overseas_bulkcollection/transactionBulkCollection")
      .then((res) => {
  ...
  const handleCurrencyChange = (value) => {
    ...
    axios
      .get(BASE_URL + "/overseas_bulkcollection/getCustomerOverseasSales", {
        params: {
          currency_id: value,
        },
      })
      .then((res) => {
        ...
        setOverseasCustomerData(uniqueCustomers);
  ...
  const fetchSalesInvoiceOrderList = (
    ...
    // setPaginationUrl(
    //   `${BASE_URL}/overseas_bulkcollection/getTransactionsByCustomer/${customerId}/${selected_currency_id}`
    // );
  ...
  const handleTransactionChange = (index, transactionId) => {
    ...
    if (transaction) {
      ...
      updatedItems[index] = {
        ...updatedItems[index],
        transactionId: transactionId, // Ensure the selected transactionId is updated
  ...
  const handleAmountValue = (value) => {
    ...
    if (parseNumber(cleanedValue) > balance_now) {
      swal({
        icon: "error",
        title: "Oppss!",
    ...
    if (confirmed) {
      try {
        const res = await axios.post(
          `${BASE_URL}/overseas_bulkcollection/addPaymentOverseas`,
          {
            items,
  ...
  const customerOptions = overseasCustomerData.map((item) => ({
              ...
              <Form.Select
                ...
                onMouseDown={(e) => {
                  if (currency_db.length === 0) {
                    ...
                    const wrapper = document.createElement("div");
                    wrapper.classList.add("center-swal-text");
                    wrapper.innerHTML = `There are no approved transactions in the <strong>Sales Invoice</strong> module that haven't already been added to <strong>Overseas Collection</strong>.`;
              ...
              <div className="row mt-4">
                ...
                <div className="col-sm d-flex flex-row align-items-end mb-2 w-100">
                  <button
                    ...
                    onClick={() => navigate("/sales/overseas-collections")}
                  >
                    Cancel
                  </button>
                  <button
...
export default OverseasBulkCollection;
