...
const Overseas_collection = ({ authrztn }) => {
  ...
  const overseasCollectionTotalPages = localStorage.getItem(
    "Overseas-Collection"
  );
  ...
  const validPage = useValidPage(overseasCollectionTotalPages);
  ...
  const handleSearch = (e) => {
    ...
    if (
      overseasCollectionData.find((item) => {
  ...
  useEffect(() => {
    ...
    if (collectionDataWithBalance?.length > 0) {
      localStorage.setItem("Overseas-Collection", pagination.totalPages);
    }
  ...
  const viewOverseasCollections = (row) => {
    navigate(
      `/sales/view-overseas-bulk-collection/${row.id}?page=${pagination.currentPage}`
    );
  };
