...
const ReturnedStatusTab = () => {
  ...
  const returnedSalesInvoicePagination = useServerPagination(
  ...
  const handleCustomerChange = (selectedOption) => {
    ...
    debounceTimer.current = setTimeout(() => {
      returnedSalesInvoicePagination.updateApiUrl(
        `${BASE_URL}/invoice/getSalesData/customer-filter`
      );
      returnedSalesInvoicePagination.updateParams({
        startDate: selectedCutOff?.from,
        endDate: selectedCutOff?.to,
        status: "Returned",
        currencyId,
  ...
  const handleSearch = (e) => {
    ...
    const isMatchingTotalAmount = returnedSalesInvoicePagination.data.some(
    ...
    <div className="container-fluid">
      ...
      <div className="w-100 mt-3">
        <div className="sales-invoice-data-table">
          <DataTable
            columns={columns}
            data={returnedSalesInvoicePagination.data}
            customStyles={customStyles}
            className="dataTable"
            onRowClicked={handleUpdateInvoice}
          />
          <PaginationControls {...returnedSalesInvoicePagination} />
        </div>
      </div>
    </div>
  );
...
export default ReturnedStatusTab;
