...
function BankTransaction({ authrztn }) {
  ...
  const fetchCutOff = () => {
    ...
    setTimeout(() => {
      reloadTable(initialCutoff, selectedAccount?.value || null);
  ...
  const columns = [
    ...
    {
      name: "Issued To",
      selector: (row) => row.account_list_id_bank_tos?.account_name || "N/A",
    },
    {
      name: "Subject From",
  ...
  const handleConfirm = async (
    ...
    try {
      ...
      if (toPay) {
        await axios
          .post(BASE_URL + "/bankTransaction/v2/confirmBankTransaction", {
            id,
            account_list_id_bank_from,
            amount,
            date,
          ...
          .then((res) => {
            if (res.status === 200) {
              swal({
                ...
                reloadTable(currentCutoff, selectedAccount?.value);
              });
  ...
  const totalIn = parseFloat(String(totals.totalWithdraw).replace(/,/g, ""));

  const totalOut = parseFloat(String(totals.totalDeposit).replace(/,/g, ""));
  ...
  const tabFilteredData = filteredData?.filter((data) => {
            ...
            <div className="col-md-6">
              <div className="card text-center">
                <div className="card-body">
                  ...
                  >
                    ₱{" "}
                    {totals?.totalWithdraw.toLocaleString("en-US", {
                      maximumFractionDigits: 2,
                      minimumFractionDigits: 2,
                    })}
                    {/* {totals?.totalWithdraw != 0
                      ? compactNumberFormat(totalIn)
                      : "0.00"} */}
                  </p>
                </div>
              </div>
            ...
            <div className="col-md-6">
              <div className="card text-center">
                <div className="card-body">
                  ...
                  >
                    ₱{" "}
                    {totals.totalDeposit?.toLocaleString("en-US", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                    {/* {totals.totalDeposit != 0
                      ? compactNumberFormat(totalOut)
                      : "0.00"} */}
                  </p>
                </div>
              </div>
