...
const OverseasExpenses = ({ authrztn }) => {
  ...
  const overseasExpenseTotalPages = localStorage.getItem("Overseas-Expense");
  const validPage = useValidPage(overseasExpenseTotalPages);
  ...
  if (authrztn.includes("OverseasExpenses-Delete")) {
    columns.push({
      name: "ACTION",
      selector: (row) => (
        <i
  ...
  const handleDeleteOverseasExpenses = async (
    localExpensesId,
    localExpensesDate,
    transaction_id
  ) => {
      ...
      if (willDelete) {
        try {
          ...
          if (response.status === 200) {
            swal({
              title: "Overseas Expenses Deleted Successfully!",
              text: "The overseas expenses has been successfully deleted.",
              icon: "success",
              button: "OK",
            }).then(() => {
            ...
            swal({
              ...
              text: `Overseas Expenses cannot be deleted as its expenses date (${expensesLocalDate}) falls within the posted cutoff period named "${CutoffName}".`,
              icon: "warning",
              button: "OK",
            });
          } else {
  ...
  const handleRowClick = (data) => {
    navigate(
      `/accounting/view-pay-expenses/overseas/${data.id}?page=${pagination.currentPage}`
    );
  };
  ...
  useEffect(() => {
    ...
    if (pagination.data.length > 0) {
      localStorage.setItem("Overseas-Expense", pagination.totalPages);
    }
  ...
  return (
    <div className="h-100 w-100 border bg-white custom-container">
      ...
      ) : authrztn.includes("OverseasExpenses-View") ? (
        <>
          <div className="w-100 p-2 d-flex flex-row justify-content-between mb-2">
            <div className="d-flex flex-column title-custom">
              <span className="fs-3">OVERSEAS EXPENSES</span>
            </div>
            <div>
              {authrztn.includes("OverseasExpenses-Add") && (
                <Link
                  to={`/accounting/pay-expenses/${"overseas"}`}
...
export default OverseasExpenses;
