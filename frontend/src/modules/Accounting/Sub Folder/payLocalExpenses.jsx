...
const LocalBulkExpenses = () => {
  ...
  const deletePayment = (item, index) => {
    ...
    // });
    setTotalPayment((prev) => prev - item.amountInputted);
  };
  ...
  const submitModal = () => {
    ...
    fetchExpensesList(foreign_url, selected_currency_id, mergedData);
  ...
  const fetchCurrency = async () => {
    const foreign =
      foreign_url.charAt(0).toUpperCase() +
      foreign_url.substring(1, foreign_url.length);
    await axios
      .get(BASE_URL + "/currency/expense-currency", {
        params: {
          foreign,
        },
      })
  ...
  const handleAmountValue = (value) => {
    ...
    if (numericValue > payableExpenses && new Date(date) <= new Date(currentDate)) {
      swal({
        title: "Oppss!",
  ...
  const toPayAmount = parseFloat(totalAmountSum); // Expenses balance
  const totalPayAmount = parseFloat(totalPayment); // Total payment
  const additionalExpense = sumAmounts("additional"); // Total additional expenses
  const deductionExpense = sumAmounts("deduction"); // Total deduction expenses

  const balance = toPayAmount + additionalExpense - deductionExpense; // Balance
  const totalPayable = additionalExpense + toPayAmount; // Expenses + Additional Expenses
  const totalWithDeductions = deductionExpense + totalPayAmount; // Deduction Expenses + Payments
  const payableExpenses = totalPayable - totalWithDeductions; // Overall expense total // prettier-ignore
  ...
  const handleSavePayment = async () => {
    ...
    dateValidation(payExpensesDate, setPayExpensesDate, "Pay Date");

    if (!payExpensesDate) {
      swal({
        title: "Required Field Missing",
        text: "Please ensure Pay Date is filled out.",
        icon: "error",
        buttons: false,
        timer: 3000,
      });
  ...
  useEffect(() => {
    ...
    setLocalExpensesData(pagination?.data?.data);
    ...
    <div className="h-100 w-100 border bg-white custom-container">
      <div className="w-100 p-2 d-flex flex-row justify-content-between">
        <div className="d-flex flex-column title-custom">
          <span className="fs-3">
            <Link to={"/accounting/local-expenses"} className="text-dark me-2">
      ...
      <div className="container-fluid mt-3">
        <div className="row p-2">
            ...
            <select
              ...
              onMouseDown={(e) => {
                if (currency_db.length === 0) {
                  e.preventDefault();
                  const foreign =
                    foreign_url.charAt(0).toUpperCase() + foreign_url.slice(1);
                  const wrapper = document.createElement("div");
                  wrapper.classList.add("center-swal-text");
                  wrapper.innerHTML = `There are no approved transactions in the <strong>Create Expenses</strong> module that haven't already been added to <strong>${foreign} Expense</strong>.`;
          ...
          <div className="col-sm">
            ...
            <CustomDatePicker
              label={"Pay Date"}
              selected={payExpensesDate ? new Date(payExpensesDate) : ""}
              handleDateChange={(date) => {
                setPayExpensesDate(date);
                dateValidation(date, setPayExpensesDate, "Pay Date");
                postedCutoffValidation(date, setPayExpensesDate, "Pay Date");
              }}
              setter={setPayExpensesDate}
                  ...
                  <Form.Group className="mb-3" controlId="date">
                    ...
                    <CustomDatePicker
                      ...
                      handleDateChange={(date) => {
                        ...
                        setIsAmountDisabled(false);
                        dateValidation(date, setDate, "Date");
                        postedCutoffValidation(date, setDate, "Date");
                      }}
                      setter={setDate}
                      CustomInput={CustomInput}
                      isRequired={true}
                ...
                <div className="col-sm">
                  ...
                  <div className="w-100 d-flex flex-row justify-content-between p-3 mt-3 total-amount-container align-items-center rounded text-white">
                    ...
                    <span>
                      ...
                      {truncateToTwoDecimals(payableExpenses)}
                    </span>
                  </div>
                </div>
                <div className="col-sm"></div>
