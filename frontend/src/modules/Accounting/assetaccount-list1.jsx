...
const AssetAccount = ({ authrztn }) => {
      ...
      if (result) {
        ...
        if (res.status === 200) {
          swal({
            ...
            reloadData();
            handleCloseSubject2Modal(); // Change this from clearInput()
          });
        } else if (res.status === 201) {
          swal({
            title: "Oppss!",
  ...
  const subject3Column = [
    ...
    },
    {
      ...
      selector: (row) => {
        return (
          <>
            {authrztn.includes("AssetAccount-Edit") && (
              <Link
                to={`/accounting/view-assetAccount/${row.id}?subject2=${selectedSubject}&items-shown=${pagination.itemsPerPage}`}
                className="btn btn-outline-primary"
              >
                View
              </Link>
            )}
            {authrztn.includes("AssetAccount-Delete") && (
              <button
  ...
  return (
    <div className="h-100 w-100 border bg-white custom-container">
      ...
      ) : authrztn.includes("AssetAccount-View") ? (
        <>
          <div className="w-100 p-2 d-flex flex-row justify-content-between mb-5">
            <div className="d-flex flex-column title-custom">
              <span className="fs-3 text-uppercase">Asset Account</span>
            </div>
          </div>
          <Tab.Container
            activeKey={activeTab}
            ...
            <Row>
              <Col sm={3}>
                <div className="d-flex justify-content-between align-items-center p-2">
                  ...
                  {authrztn.includes("AssetAccount-Add") && (
                    <i
                ...
                {/* <Form.Group className="d-flex flex-row align-items-center justify-content-center mt-3">
                  ...
                  {authrztn.includes(
                    isEditing ? "AssetAccount-Edit" : "AssetAccount-Add"
                  ) && (
                    <Button
                            ...
                            .map((item) => (
                              <tr key={item.id}>
                                ...
                                <td style={{ padding: "15px" }}>
                                  {authrztn.includes("AssetAccount-Edit") && (
                                    <Link
                                      to={`/accounting/view-assetAccount/${item.id}`}
                                      className="btn btn-outline-primary"
                                    >
                                      Edit
                                    </Link>
