...
function ReturnEarnings({ authrztn, roleType }) {
  ...
  const handleAmountToAddFunds = (value) => {
    ...
    const accountAmount = accountListSub3.find(
      (account) => String(account.id) === String(selectedSubject3)
    )?.amount;
    if (
          ...
          <Button
            className={
              roleType?.includes("Management")
                ? "btn d-flex align-items-center title-button"
                : "d-none"
            }
            variant="secondary"
