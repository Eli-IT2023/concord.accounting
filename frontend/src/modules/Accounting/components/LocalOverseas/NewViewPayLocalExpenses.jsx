...
const NewViewPayLocalExpenses = ({
  ...
  const submitModal = () => {
    ...
    fetchAvailableExpenses(mergedData);
    setSelectedRowTransactions(mergedData);

    setIsAddedTransactions((prev) => {
      return prev.filter((item) => !mergedData.includes(item.transaction_id));
    });
  ...
  const deleteItem = async (item, index) => {
    ...
    if (item.isAdded === true) {
      await axios.put(
        `${BASE_URL}/paylocalexpenses/deleteOrderListTransaction`,
        {
          id,
          idToRemove: item.id,
        }
      ...
      setTransactionToGetBack((prev) => [...prev, item.id]);
      setIdToAdd((prev) => [...prev, item.id]);
      setId(id);
      setRoute("paylocalexpenses");
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => (
        <>
          <div className="d-none">{row.transaction_id}</div>
          <div>{row.client_transaction_id}</div>
        </>
      ),
    },
    {
  ...
  const fetchAvailableExpenses = async (selectedTransactions = []) => {
  ...
  useEffect(() => {
    ...
    if (pagination?.data?.items) {
      // Skip if all selectedRows already exist in Expense List Table
      const existingIds = transactions.map((item) => item.transaction_id);
      const isFullySynced =
        selectedRowTransactions.every((id) => existingIds.includes(id)) &&
        transactions.length === selectedRowTransactions.length;

      if (isFullySynced) return;

      const newItems = pagination.data.items.filter(
        (item) =>
          selectedRowTransactions.includes(item.transaction_id) &&
  ...
  const handleAddNewExpense = () => {
    fetchAvailableExpenses(selectedRowTransactions);
    ...
    swal({
      ...
      if (confirmed) {
        ...
        navigator.sendBeacon(
          `${BASE_URL}/paylocalexpenses/getBackOrderListTransaction`,
          blob
        );
      ...
      if (confirmed) {
        ...
        try {
          const balance =
            ...
            totalDeductionExpenses;

          // For expense journal
          const currency = currency_db.find(
            (c) => c.id === selected_currency_id
          );

          axios
            .post(`${BASE_URL}/paylocalexpenses/bulk/approve`, {
              id,
              foreign_url,
              processedPayments,
            ...
            .then((res) => {
              if (res.status === 201) {
                ...
                swal({
                  ...
                  //   navigate(
                  //     `/accounting/local-expenses?page=${searchParams.get(
                  //       "page"
                  //     )}`
                  //   );
                  // } else {
                  //   navigate(
                  //     `/accounting/overseas-expenses?page=${searchParams.get(
                  //       "page"
                  //     )}`
                  //   );
                  // }
  ...
  const totalAdditionalExpenses = addDeducts
    .filter((data) => data.type_expenses === "additional")
    .reduce((acc, curr) => acc + (curr.amount || 0), 0);

  const totalDeductionExpenses = addDeducts
    .filter((data) => data.type_expenses === "deduction")
    .reduce((acc, curr) => acc + (curr.amount || 0), 0);
  ...
  const handleUpdate = () => {
    ...
    const totalAdditionalExpenses_forUpdate = addDeducts
      .filter((data) => data.type_expenses === "additional")
      .reduce((acc, curr) => acc + (curr.amount || 0), 0);

    const totalDeductionExpenses_forUpdate = addDeducts
      .filter((data) => data.type_expenses === "deduction")
      .reduce((acc, curr) => acc + (curr.amount || 0), 0);
        ...
        try {
          ...
          for (const expense of Object.values(expenses.additional)) {
            expensesToSubmit.push({
              ...
              amount: parseFloat(String(expense.amount).replace(/,/g, "")) || 0,
          ...
          axios
            .post(`${BASE_URL}/paylocalexpenses/update`, null, {
              params: {
                ...
                totalAmountSum: parseFloat(
                  expensesTotalAmount +
                    totalAdditionalExpenses_forUpdate -
                    totalDeductionExpenses_forUpdate
                ),
                currencyRate,
              },
            })
  ...
  const isInputDisabled =
    parseFloat(payableBalance) === 0 ||
    (parseFloat(payableBalance) === 0 && !isEditing) ||
    (status === "For-Approval" && !isEditing);

  // Subject 2 Payment Method Options for dropdown select
  ...
  // Subject 3 Payment Method Options for dropdown select
    ...
    if (form.checkValidity() === false) {
      ...
      swal({
        ...
        if (confirmed) {
          ...
          // Handle payment for pending transactions
          if (isForApproval) {
          ...
          // Handle payment for approved transactions
          if (!isForApproval) {
            try {
              const res = await axios.post(
                `${BASE_URL}/paylocalexpenses/bulk/payments`,
                {
                  newPayment,
                  payBulkId: id,
                }
  ...
  const renderExpenseTable = (type) => {
    ...
    const subject1AddDeductOptions = [
      "Account-List",
      "Liabilities Account",
      "Asset Account",
    ...
    const subject2AddDeductOptions = (row) =>
    ...
    const subject3AddDeductOptions = (row) =>
      row.subject3Options.map((data) =>
        row.subject1 === "Loan_Unique"
          ? {
              value: data.id,
              label: data.loan_name,
            }
          : { value: data.id, label: data.account_name }
      );
      ...
      <tbody>
        {Object.entries(expenses[type]).map(([rowId, row]) => (
          <tr key={rowId}>
            <td>
              <Select
                options={subject1AddDeductOptions}
                value={subject1AddDeductOptions.find(
                  (item) => item?.value === row.subject1
                )}
                ...
                styles={selectCustomStyles(row.subject1)}
                required
              />
            </td>
            <td>
              <Select
                options={subject2AddDeductOptions(row)}
                value={subject2AddDeductOptions(row).find(
                  (item) => item?.value === row.subject2
                )}
                onChange={(selectedOption) =>
                ...
                styles={selectCustomStyles(row.subject2)}
                isDisabled={row.disabledSubject2}
                required
              />
            </td>
            <td>
              <Select
                options={subject3AddDeductOptions(row)}
                value={subject3AddDeductOptions(row).find(
                  (item) => item?.value === row.subject3
                )}
                onChange={(selectedOption) => {
                  const selectedValue = selectedOption?.value;

                  const selectedOptions = row.subject3Options.find(
                    (option) => option.id == selectedValue
                  );

                  const currAmount = selectedOptions?.amount || 0;
                ...
                menuPortalTarget={document.body}
                isDisabled={row.disabledSubject3}
                placeholder={`Select Subject 3`}
                styles={selectCustomStyles(row.subject3)}
                required
              />
            </td>
            <td>
  ...
  const CustomInput = React.forwardRef(
    ({ value, onClick, generateYears, disabled }, ref) => (
      <input
        type="text"
    ...
    <div className="h-100 w-100 border bg-white custom-container">
      <div className="w-100 p-2 d-flex flex-row justify-content-between">
        <div className="w-100 d-flex flex-row justify-content-between">
          <span className="fs-3">
            <Link
              to={`${`/accounting/local-expenses?page=${searchParams.get(
                "page"
              )}`}`}
            ...
            NEW {foreignExpenses} EXPENSES DETAILS
          </span>
      ...
      <div className="container-fluid mt-3">
        <div className="row p-2">
          ...
          <div className="col-sm">
            ...
            <CustomDatePicker
              label={"Pay Date"}
              selected={payDate ? new Date(payDate) : ""}
              handleDateChange={(date) => {
                setPayDate(date);
                dateValidation(date, setPayDate, "Transaction Date");
                postedCutoffValidation(date, setPayDate, "Transaction Date");
              }}
              setter={setPayDate}
              ...
              <div className="input-group">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (
                      localExpensesData.find(
                        (item) =>
                          Number(item.totalAmount.toPrecision(17))
                            .toString()
                            .includes(raw) ||
                          item.totalAmount.toFixed(17).includes(raw)
                      )
                    ) {
                ...
                <Form validated={validated} onSubmit={handleAddPayment}>
                  ...
                  <Form.Group className="mb-3">
                    ...
                    <Select
                      ...
                      onMenuOpen={() => {
                        if (subject2DataList.length === 0) {
                          swal({
                            icon: "warning",
                            title: "Warning",
                            text: "No Subject 2 found. Please create a Subject 2 account first.",
                          });
                          return;
                        }
                      }}
                      menuIsOpen={
                        subject2DataList?.length === 0 ? false : undefined
                      }
                  ...
                  <Form.Group className="mb-3" controlId="accountName">
                    ...
                    <Select
                      ...
                      onMenuOpen={() => {
                        if (subject3DataList.length === 0) {
                          swal({
                            icon: "warning",
                            title: "Warning",
                            text: "No Subject 3 found. Please create a Subject 3 account first.",
                          });
                          return;
                        }
                      }}
                      menuIsOpen={
                        subject3DataList.length === 0 ? false : undefined
                      }
              ...
              <div className="row mt-5">
                <div className="col-sm">
                  ...
                  <div className="d-flex justify-content-between">
                    ...
                    <span className="text-secondary">
                      {totalAdditionalExpenses.toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </span>
                  ...
                  <div className="d-flex justify-content-between">
                    ...
                    <span className="text-secondary">
                      {totalDeductionExpenses.toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </span>
                        ...
                        {authrztn.includes(approvePermission) && (
                          <>
                            <div className="d-flex flex-column gap-2">
                              ...
                              <div>
                                {(authrztn.includes("LocalExpenses-Edit") ||
                                  authrztn.includes("LocalExpenses-Approve")) &&
                                  isCutoffPosted && (
                                    <div>
                                      <p className="text-danger">
                                        Action is prohibited as the Expenses
...
export default NewViewPayLocalExpenses;
