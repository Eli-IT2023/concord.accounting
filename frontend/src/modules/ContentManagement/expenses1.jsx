...
const Expenses1 = ({ authrztn }) => {
  ...
  const [incomeStatementCheckBox, setIncomeStatementCheckbox] = useState(false);
  ...
  const handleUnarchive = async (rowExpensesId) => {
    swal({
  ...
  const deleteExpensesRow = async (rowExpensesId) => {
    swal({
      ...
      if (willDelete) {
        try {
          const response = await axios.delete(
            ...
            { data: { expensesId: rowExpensesId, userLoggedID } }
          );
          if (response.status === 200) {
            swal({
  ...
  const handleUpdateModal = async (data) => {
    ...
    setIncomeStatementCheckbox(data.isForIncomeStatement);
  };
  ...
  if (authrztn.includes("ExpensesType1-Delete")) {
    columns.push({
      name: "Action",
      selector: (row) => (
        <>
          {row.isArchive ? (
            <>
              <Button
                variant="danger"
                onClick={() => handleUnarchive(row.expenses_one_id)}
              >
                Unarchive
              </Button>
            </>
            ...
            <>
              <Button
                variant="danger"
                onClick={() => deleteExpensesRow(row.expenses_one_id)}
              >
                Archive
              </Button>
            </>
              ...
              <div className="col-sm">
                <Form.Group className="d-flex">
                  <div className="me-3 mt-1">
                    <input
                      type="checkbox"
                      checked={incomeStatementCheckBox}
                      onChange={(e) => {
                        setIncomeStatementCheckbox(e.target.checked);
                      }}
                      name="income-statement-checkbox"
                      id="income-statement-checkbox"
                      className="form-check-input border border-secondary"
                    />
                  </div>
                  <Form.Label
                    className="fs-5 text-uppercase"
                    htmlFor="income-statement-checkbox"
                  >
                    Income statement
                    {/* Tooltip  */}
                    <OverlayTrigger
                      ...
                      overlay={
                        <Tooltip className="ms-1">
                          If checked, this will be reflected in the income
                          statement (Total Operating Expenses)
                        </Tooltip>
                      }
                    >
                      {({ ref, ...triggerHandler }) => (
        ...
        <Form noValidate validated={validated} onSubmit={handleFormUpdate}>
          ...
          <Modal.Body>
            ...
            <div className="row mt-3">
              <div className="col-6">
                <Form.Group controlId="exampleForm.ControlInput1">
                  <Form.Label className="fs-5">EXPENSES TYPE NAME</Form.Label>
                  <Form.Control
                    className="p-3"
                    required
                    type="text"
